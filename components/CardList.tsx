...
function CardView({ card, onDelete, onUpdate, folders, onMove }: {
  card: any;
  onDelete: (id: string) => void;
  onUpdate: (id: string, patch: { translation?: string; exampleSentences?: string[] }) => void;
  ...
  const [translation, setTranslation] = React.useState(card.translation as string);
  const [examples, setExamples] = React.useState<string[]>(card.exampleSentences as string[]);
  ...
  const editError = (() => {
    ...
    if (examples.some((e: string) => !(e || "").trim())) return "Example sentences cannot be empty.";
    return "";
  })();
  const { user } = useUser();
  ...
  return (
    <div className="border rounded p-3 bg-white shadow-sm">
      ...
      <div className="flex items-center gap-2 mb-2">
        <button className="text-xs underline" onClick={() => setEditOpen(true)}>Edit</button>
        <button className="text-xs underline" onClick={() => setMoveOpen(true)}>Move</button>
      </div>
      <Modal
        open={editOpen}
        title="Edit Card"
        ...
        footer={
          <div className="flex justify-end gap-2">
            <Button variant="outline" size="sm" onClick={() => setEditOpen(false)} disabled={savingEdit}>Cancel</Button>
            <Button
              size="sm"
      ...
      >
        <div className="space-y-3">
          <div>
            ...
            <input className="w-full border rounded px-2 py-1" value={translation} onChange={(e) => setTranslation(e.target.value)} />
          </div>
          <div>
            <div className="text-xs text-neutral-600 mb-1">Examples (3)</div>
            {examples.map((ex, i) => (
              <input key={i} className="w-full border rounded px-2 py-1 mb-1" value={ex} onChange={(e) => {
                const next = [...examples];
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-end gap-2">
            ...
            <Button size="sm" disabled={moveTarget === card.folderId} onClick={() => { onMove(card._id, moveTarget); setMoveOpen(false); }}>Move</Button>
          </div>
        }
      >
        <select className="w-full border rounded px-2 py-2" value={moveTarget} onChange={(e) => setMoveTarget(e.target.value)}>
          {folders.map((f) => (
...
export default function CardList() {
  ...
  const deleteCard = useMutation("cards:deleteCard");
  const updateCard = useMutation("cards:updateCard");
  const moveCard = useMutation("cards:moveCard");
  ...
  const cards = useQuery(
    q
      ? "cards:searchCards"
      : "cards:listCardsByFolder",
    selectedFolderId
      ? (q
          ? { q, folderId: selectedFolderId as any }
          : { folderId: selectedFolderId as any })
      : "skip"
  );
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteCard({ cardId: id as any });
  ...
  const handleUpdate = async (
    ...
    try {
      await updateCard({
        ...
        exampleSentences: patch.exampleSentences as any,
      });
  ...
  const handleMove = async (id: string, folderId: string) => {
    try {
      await moveCard({ cardId: id as any, folderId: folderId as any });
