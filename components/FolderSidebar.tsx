...
export default function FolderSidebar() {
  ...
  const createFolder = useMutation("folders:createFolder");
  const deleteFolder = useMutation("folders:deleteFolder");
  ...
  const nameError = (() => {
    ...
    if (folders && folders.some((f) => f.name.toLowerCase() === n.toLowerCase()))
    ...
    <aside className="w-64 border-r bg-white flex flex-col">
      ...
      >
        <p className="text-sm text-neutral-700">
          Are you sure you want to delete “{deleteTarget?.name}”? This action requires the folder to be empty.
        </p>
      </Modal>
    </aside>
  );
